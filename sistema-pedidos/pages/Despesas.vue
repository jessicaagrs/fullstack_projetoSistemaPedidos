    ...
    <v-form v-model="valid">
      <v-container>
        <v-row class="d-flex justify-center align-center">
          ...
          <v-col cols="12" md="4">
            <v-text-field
              v-model="tipoDespesa.grupo"
              label="Grupo:"
            ></v-text-field>
          </v-col>
          <v-col cols="12" md="4">
            <v-btn color="primary" @click="salvarTipoDespesa"> Salvar </v-btn>
          </v-col>
        </v-row>
      </v-container>
    </v-form>
    ...
    async salvarTipoDespesa() {
      try {
        if (this.tipoDespesa.descricao != "" && this.tipoDespesa.grupo != "") {
          let contemElemento = this.tipoDespesas.some(
        ...
        if (this.selected.length == 1) {
          if (this.selected[0].id) {
            ...
            await this.$axios.delete(
              `https://localhost:7054/TipoDespesa/${id}`
            );
            await this.getTipoDespesas();
          }
        } else {
