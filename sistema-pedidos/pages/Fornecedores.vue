    ...
    <v-form v-model="valid">
      <v-container>
        <v-row class="d-flex justify-sparce-between align-center">
          ...
          <v-col cols="12" md="4">
            <v-btn color="primary" @click="salvarFornecedor"> Salvar </v-btn>
          </v-col>
        </v-row>
      </v-container>
    </v-form>
    ...
    async salvarFornecedor() {
      try {
        if (
          ...
          let contemElemento = this.fornecedores.some(
          ...
          this.errorMessage =
            "Os campos razão social, CNPJ e tipo da despesa são obrigatórios.";
        }
      } catch (error) {
        this.errorMessage = error;
      }
    ...
    async getTipoDespesas() {
      try {
        const response = await this.$axios.get(
          "https://localhost:7054/TipoDespesa"
        );
